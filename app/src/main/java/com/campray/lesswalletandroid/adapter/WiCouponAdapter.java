 ...
 * @author Phills
 */
public class WiCouponAdapter extends BaseRecyclerAdapter<Coupon> {
    public WiCouponAdapter(Context context, int layoutResourceId, Collection<Coupon> datas) {
...
//                break;
     ...
     */
    @Override
    public void bindView(final BaseRecyclerHolder holder, Coupon coupon, final int position) {
        if(coupon!=null) {
...
//                        }
                        final Coupon delCoupon=WiCouponAdapter.this.getItem(position);
                        List<Coupon>  couponList=delCoupon.getProduct().getCoupons();
                        final int couponSize=couponList.size();
                        ...
                        CouponModel.getInstance().delCouponsFromServer(idsStr, new OperationListener<Coupon>() {
                            ...
                            public void done(Coupon obj, AppException exception) {
                                if(exception==null) {
                                    ...
                                    WiCouponAdapter.this.remove(startPos);
                                    dialog.cancel();
                                }
                                else{
