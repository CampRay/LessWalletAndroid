...
import com.campray.lesswalletandroid.qrcode.camera.open.OpenCameraInterface;
...
public final class CameraManager {
  ...
  public synchronized Rect getFramingRectInPreview() {
    if (framingRectInPreview == null) {
      ...
      //摄相屏是否竖屏
      boolean cameraIsPortrait = (cameraResolution.x < cameraResolution.y);
      //扫码屏是否竖屏
      boolean screenIsPortrait = (screenResolution.x < screenResolution.y);
      //如果屏幕方向一致
      if (cameraIsPortrait == screenIsPortrait) {
  ...
  public PlanarYUVLuminanceSource buildLuminanceSource(byte[] data, int width, int height) {
    ...
    // 截取取码框图片进行解码 Go ahead and assume it's YUV rather than die.
